...
/*   Updated: 2024/09/05 04:31:23 by jmakkone         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static void	reorganize_parsed_cmd(t_shell *shell, int redir_count)
{	
	char **new_cmd;
    int	i;
	int	j;
	...
	i = 0;
	j = 0;
    new_cmd = malloc(sizeof(char *) * (arr_len(shell->parsed_cmd) + 1 - (redir_count * 2)));
    while (shell->parsed_cmd[i])
	{
		if (!ft_strcmp(shell->parsed_cmd[i], "<") || !ft_strcmp(shell->parsed_cmd[i], "<<") \
			|| !ft_strcmp(shell->parsed_cmd[i], ">") || !ft_strcmp(shell->parsed_cmd[i], ">>"))
			i += 2;
		else
...
void	parse_redirections(t_shell *shell)
{
    ...
    reorganize_parsed_cmd(shell, redir_count);
}
